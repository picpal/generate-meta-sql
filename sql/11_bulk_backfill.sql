...
--
--   LISTAGG는 4000 byte에서 잘린다. 대상이 많으면 스키마·서비스 단위로
--   나눠 여러 배치로 실행한다(각 배치의 CHANGE_REASON 접미사를 다르게 둘 것).
...
SELECT LISTAGG(TABLE_ID, ',') WITHIN GROUP (ORDER BY TABLE_ID) AS IDS
  FROM TB_META_TABLE
 WHERE SERVICE_CD = 'UNASSIGNED'
...
-- (4) HIST 적재 → §11.1 (4)의 INSERT를 복사해
--     CHANGE_REASON만 'BACKFILL:OWNER_EMP_ID:&BATCH' 로 바꿔 실행
--     (WHERE TABLE_ID IN (&IDS) 는 그대로 둔다)
...
--     (아래는 주민등록번호 예시. 다른 분류는 WHERE와 값만 바꿔 반복)
UPDATE TB_META_COLUMN
 ...
 WHERE STATUS_CD = 'ACTIVE'
   AND REGEXP_LIKE(COLUMN_NAME, 'RRN|JUMIN|SSN', 'i');
...
SELECT SEQ_META_HIST_ID.NEXTVAL, 'U', SYSTIMESTAMP, '&EMP_ID',
  ...
  FROM TB_META_COLUMN
 WHERE STATUS_CD = 'ACTIVE'
   AND REGEXP_LIKE(COLUMN_NAME, 'RRN|JUMIN|SSN', 'i');   -- ← (3)과 동일 조건

COMMIT;

-- (5) 테이블 PCI_YN 롤업 — PCI 컬럼을 하나라도 가진 테이블은 PCI_YN='Y'
--     대상 ID 고정 (UPDATE 후에는 PCI_YN='N' 조건이 성립하지 않는다)
SELECT LISTAGG(t.TABLE_ID, ',') WITHIN GROUP (ORDER BY t.TABLE_ID) AS IDS
...
UPDATE TB_META_TABLE
 ...
 WHERE TABLE_ID IN (&IDS);
-- HIST 적재 → §11.1 (4)를 CHANGE_REASON='BACKFILL:PCI_ROLLUP:&BATCH' 로 바꿔 실행
COMMIT;
...
-- HIST 적재 → §11.1 (4)를 복사해
--   CHANGE_REASON = 'BACKFILL:RETENTION:&BATCH'
--   WHERE         = SERVICE_CD = 'MEMBER' AND PCI_YN = 'Y'   (위와 동일)
-- 로 바꿔 실행
COMMIT;
...
-- (3) HIST 적재 → §11.3 (4)를 CHANGE_REASON='BACKFILL:DEFAULT_VALUE:&BATCH' 로 바꿔 실행
-- COMMIT;
