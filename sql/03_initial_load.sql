...
--   ALL_TABLES → VIEW_YN='N', ALL_VIEWS → VIEW_YN='Y' 로 적재
-- =====================================================================
...
SELECT
    ...
    SUBSTR(c.COMMENTS, 1, 2000),          -- ALL_TAB_COMMENTS (2000자 절단: ORA-12899 방지)
    'N',                                  -- VIEW_YN
    'UNASSIGNED',                         -- 이후 매핑
    'SYSTEM',                             -- 담당자 미지정
    'N','N','N',
...
SELECT
    ...
    v.VIEW_NAME,
    NULL,
    SUBSTR(c.COMMENTS, 1, 2000),          -- ALL_TAB_COMMENTS는 뷰 코멘트도 포함
    'Y',                                  -- VIEW_YN
    'UNASSIGNED',
    'SYSTEM',
    'N','N','N',
...
WHERE v.OWNER IN ('SVC1','SVC2'/* 대상 스키마 목록 */)
  ...
  AND NOT EXISTS (
        SELECT 1 FROM TB_META_TABLE m
         WHERE m.SCHEMA_NAME = v.OWNER
           AND m.TABLE_NAME  = v.VIEW_NAME
      )
;
...
SELECT
    ...
    tc.COLUMN_ID,
    NULL,
    SUBSTR(cc.COMMENTS, 1, 2000),         -- ALL_COL_COMMENTS (2000자 절단: ORA-12899 방지)
